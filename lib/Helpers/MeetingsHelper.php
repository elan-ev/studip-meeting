...
class MeetingsHelper
{
    ...
    public static function performJoin($room_id, $cid, $lobby_redirect = [], $error_redirect = [])
    {
        ...
        if (!$GLOBALS['perm']->have_studip_perm('user', $cid) || ($meetingCourse->group_id && !RoomManager::checkGroupPermission($meetingCourse->group_id, $cid))) {
        ...
        if (!$allow_course_type || !$active_server) {
            $err                          = !$allow_course_type ? 'course-type' : 'server-inactive';
        ...
        if (isset($features['room_anyone_can_start'])
            ...
            && !$GLOBALS['perm']->have_studip_perm('tutor', $cid)
        ) {
        ...
        if ($GLOBALS['perm']->have_studip_perm('tutor', $cid) || $meeting->join_as_moderator) {
